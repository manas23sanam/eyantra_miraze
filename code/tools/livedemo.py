...
from core.exercise_utils import load_config, select_exercise, get_exercise_components, process_frame_logic, SessionTracker
FRAME_WIDTH = 640
FRAME_HEIGHT = 480
FPS = 30
...
def draw_hud(image, exercise_name, current_val, metric_name, is_correct, latency_ms, fps, cue_text, rep_count, state_text):
    ...
    cv2.putText(image, exercise_name, (15, 33), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
    status_color = color_for_state(is_correct)
    cv2.putText(image, cue_text, (w - 380, 33), cv2.FONT_HERSHEY_SIMPLEX, 0.7, status_color, 2, cv2.LINE_AA)
    if current_val is not None:
        val_text = f'{metric_name}: {current_val:.1f}'
        cv2.putText(image, f'{val_text} | State: {state_text.upper()}', (15, h - 80), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(image, f'REPS: {rep_count}', (15, h - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2, cv2.LINE_AA)
    else:
        cv2.putText(image, 'No pose detected', (15, h - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2, cv2.LINE_AA)
    cv2.putText(image, f'Latency: {latency_ms:.1f} ms   FPS: {fps:.1f}', (15, h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2, cv2.LINE_AA)
...
def main():
    ...
    try:
        while True:
            ...
            smoothed_metric, metric_name, is_correct, cue_text, state_text, rep_count = process_frame_logic(result, rep_counter, angle_smoother, selected_key, exercise_config)
            ...
            draw_hud(display_image, exercise_name, smoothed_metric, metric_name, is_correct, avg_latency, live_fps, cue_text, rep_count, state_text)
