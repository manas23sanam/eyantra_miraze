...
class ShoulderCircleDetector(PoseGeometryBase):
    ...
    def process_frame(self, color_image: np.ndarray, depth_frame=None, depth_image=None) -> Optional[dict]:
        ...
        return {'angle_deg': angle_deg, 'chest_width': chest_width, 'shrug_height': shrug_height, 'landmarks': results.pose_landmarks}
...
class ShoulderCircleExercise(BaseExercise):
    def __init__(self, config, intrinsics):
        super().__init__(config, intrinsics)
        self.detector = ShoulderCircleDetector(depth_intrinsics=intrinsics)
        self.counter = ShoulderCircleRepCounter()
    ...
    def process_frame(self, color_image, depth_frame, depth_image):
        ...
        if smoothed_metric is not None:
            state_text, rep_count, just_completed = self.counter.update(smoothed_metric)
            is_correct = True
