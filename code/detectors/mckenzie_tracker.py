...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        try:
            ...
            cv2.putText(black_screen, f'TRACKING SIDE: {active_side}', (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2, cv2.LINE_AA)
            y_offset = 90
            for text in corrections:
                cv2.putText(black_screen, text, (30, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, text_color, 2, cv2.LINE_AA)
                y_offset += 30
            cv2.putText(black_screen, f'REPS: {rep_count}', (frame.shape[1] - 180, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
        except Exception as e:
            print(f'MATH ERROR: {e}')
    else:
        cv2.putText(black_screen, 'Waiting for full body (Need strict side-profile)...', (50, frame.shape[0] // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
    cv2.imshow(window_name, black_screen)
    if cv2.waitKey(10) & 255 in [27, ord('q')]:
        break
cap.release()
