...
class SitToStandDetector(PoseGeometryBase):
    ...
    def process_frame(self, color_image, depth_frame, depth_image, side='auto'):
        ...
        left_angle = self.angle_between(left_hip, left_knee, left_ankle) if left_hip is not None and left_knee is not None and (left_ankle is not None) else None
        right_angle = self.angle_between(right_hip, right_knee, right_ankle) if right_hip is not None and right_knee is not None and (right_ankle is not None) else None
        ...
        if chosen_angle is None:
            return None
        return {'hip_knee_ankle_angle_deg': chosen_angle, 'side_used': chosen_side, 'landmarks': results.pose_landmarks}
