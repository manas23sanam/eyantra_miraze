...
INSTRUCTION_LINES = ['MINI SQUAT - How to do it:', '1. Stand with feet shoulder-width apart', '2. Bend your knees slightly, like sitting back into a chair', "3. Keep your back straight, don't lean forward", "4. Go down a little, then stand back up - that's one rep", '', "Press SPACE when you're ready to begin"]
...
def draw_hud(image, exercise_name, knee_angle, is_correct, latency_ms, fps, cue_text, rep_count):
    ...
    if knee_angle is not None:
        cv2.putText(image, f'Knee angle: {knee_angle:.1f} deg', (20, h - 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
    else:
...
def main():
    ...
    try:
        while True:
            ...
            draw_hud(display_image, EXERCISE_NAME, knee_angle, is_correct, avg_latency, live_fps, cue_text, rep_count)
            cv2.imshow(WINDOW_NAME, display_image)
    finally:
        pipeline.stop()
        detector.close()
