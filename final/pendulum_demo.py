...
INSTRUCTION_LINES = ['PENDULUM EXERCISE - How to do it:', '1. Bend forward at the hips (torso angle 100-145 degrees)', '2. Let one arm hang down vertically', '3. Swing the hanging arm back and forth like a pendulum', '4. Keep the torso still while swinging the arm', '', "Press SPACE when you're ready to begin"]
...
def draw_hud(image, exercise_name, torso_bend_angle, swing_range, swing_status, swing_cycles, is_correct, latency_ms, fps):
    ...
    if torso_bend_angle is not None:
        cv2.putText(image, f'Torso Bend: {torso_bend_angle:.1f} deg (Target: 100-145)', (20, h - 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
        status_str = 'SWINGING' if swing_status else 'STILL'
        cv2.putText(image, f'Swing Range: {swing_range:.1f} deg | Status: {status_str}', (20, h - 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
    else:
...
def main():
    ...
    try:
        while True:
            ...
            draw_hud(display_image, EXERCISE_NAME, torso_bend, swing_range, is_swinging, swing_cycles, is_correct, avg_latency, live_fps)
            cv2.imshow(WINDOW_NAME, display_image)
    finally:
        pipeline.stop()
        detector.close()
