...
INSTRUCTION_LINES = [
    "UPPER TRAPEZIUS STRETCH - How to do it:",
    "1. Stand straight and keep your shoulders level",
    "2. Tilt your head to one side (at least 20 degrees)",
    "3. Keep your torso straight (lean less than 15 degrees)",
    "4. Hold the stretch for 3 seconds to count one rep",
    "",
...
def draw_hud(image, exercise_name, neck_tilt, direction, hold_timer, stretch_reps, is_correct, latency_ms, fps):
    ...
    # Bottom-left: neck tilt and hold timer
    if neck_tilt is not None:
        dir_str = direction.upper() if direction else "NONE"
        cv2.putText(image, f"Neck Tilt: {neck_tilt:.1f} deg | Direction: {dir_str}", (20, h - 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(image, f"Hold Timer: {hold_timer:.1f}s / 3.0s", (20, h - 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
    else:
...
def main():
    ...
    try:
        while True:
            ...
            draw_hud(display_image, EXERCISE_NAME, neck_tilt, direction, hold_timer,
                     stretch_reps, is_correct, avg_latency, live_fps)
