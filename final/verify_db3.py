...
# =====================================================================
# CUSTOM TRACKERS FOR VERIFICATION
# =====================================================================
...
class CustomUpperTrapeziusTracker:
    ...
    def update(self, neck_tilt_deg, torso_lean_deg, current_time):
        is_correct_posture = (neck_tilt_deg is not None and neck_tilt_deg >= self.tilt_threshold and
                              torso_lean_deg is not None and torso_lean_deg < self.max_torso_lean)
...
def draw_skeleton_with_feedback(image, landmarks_proto, is_correct):
    ...
    mp.solutions.drawing_utils.draw_landmarks(
        image, landmarks_proto, mp.solutions.pose.POSE_CONNECTIONS,
...
def main():
    ...
    db3_dir = r"C:\Users\BIT\OneDrive\Desktop\software mirror\new"
    ...
    for file_path in db3_files:
        ...
        if exercise is None:
            # Skip non-exercise db3 files silently or with brief log
            continue

        # Expected label: Correct Form or Wrong Form
        ...
        # Initialize detector and tracker based on exercise
        ...
        if exercise == "shoulder_circle":
            ...
            sit_to_stand_checklist = SitToStandChecklist(
                leg_stand_threshold=160.0, leg_sit_threshold=110.0,
                back_min_angle=150.0, arm_raise_min=70.0, arm_raise_max=110.0
            )
            ...
            while True:
                ...
                if exercise == "shoulder_circle":
                    ...
                    if result is not None:
                        ...
                        # Frame is correct if checklist passes (at least half of measured checks OK)
                ...
                if args.show_ui:
                    ...
                    # Renders simple visual overlay
                    h, w = display_image.shape[:2]
                    cv2.putText(display_image, f"{exercise.upper()} | Reps: {detected_reps}", (20, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
                    status_text = "CORRECT" if is_correct_frame else "WRONG"
                    status_color = (0, 200, 0) if is_correct_frame else (0, 0, 230)
                    cv2.putText(display_image, status_text, (20, 60),
        ...
        results_table.append({
            ...
            "Frame rate": f"{correct_frame_rate_pct:.1f}%",
            "Result": result_pass
        })
