...
def draw_teach_banner(image, seconds_remaining):
    ...
    cv2.putText(image, 'WATCH: Correct Sit to Stand Form', (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(image, f'Live demo starts in {int(seconds_remaining)}s  (press SPACE to skip)', (20, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 200, 255), 2, cv2.LINE_AA)
...
def draw_hud(image, checklist_result, latency_ms, fps, rep_count, correct_rep_count):
    ...
    cv2.putText(image, f'Reps: {correct_rep_count}/{rep_count}', (w - 480, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
    score_color = COLOR_GREEN if checklist_result.get('passed', 0) == checklist_result.get('total_measured', 0) and checklist_result.get('total_measured', 0) > 0 else COLOR_AMBER
    cv2.putText(image, f"Score: {checklist_result['score_text']}", (w - 260, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.9, score_color, 2, cv2.LINE_AA)
    y = h - 110
    for name, key in [('Legs', 'legs'), ('Back', 'back'), ('Arms', 'arms')]:
        ...
        y += 30
    cv2.putText(image, f'Latency: {latency_ms:.1f} ms   FPS: {fps:.1f}', (20, h - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2, cv2.LINE_AA)
...
def draw_summary_screen(image, total_reps, correct_reps, fail_counts):
    ...
    cv2.putText(image, 'SESSION COMPLETE', (int(w * 0.5) - 220, int(h * 0.18)), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 255), 3, cv2.LINE_AA)
    cv2.putText(image, f'Total reps: {total_reps}', (int(w * 0.5) - 180, int(h * 0.32)), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(image, f'Correct reps: {correct_reps}', (int(w * 0.5) - 180, int(h * 0.4)), cv2.FONT_HERSHEY_SIMPLEX, 0.9, COLOR_GREEN, 2, cv2.LINE_AA)
    acc_color = COLOR_GREEN if accuracy >= 70 else COLOR_AMBER if accuracy >= 40 else COLOR_RED
    cv2.putText(image, f'Accuracy: {accuracy:.1f}%', (int(w * 0.5) - 180, int(h * 0.5)), cv2.FONT_HERSHEY_SIMPLEX, 1.1, acc_color, 3, cv2.LINE_AA)
    cv2.putText(image, 'Most common issue(s):', (int(w * 0.5) - 180, int(h * 0.62)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
    if sum(fail_counts.values()) == 0:
        cv2.putText(image, '  None - great form throughout!', (int(w * 0.5) - 180, int(h * 0.69)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, COLOR_GREEN, 2, cv2.LINE_AA)
    else:
        sorted_fails = sorted(fail_counts.items(), key=lambda x: -x[1])
        y = int(h * 0.69)
        for name, count in sorted_fails:
            if count > 0:
                cv2.putText(image, f'  {name}: flagged in {count} frame(s)', (int(w * 0.5) - 180, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, COLOR_RED, 2, cv2.LINE_AA)
                y += 32
    cv2.putText(image, "Press 'q' to close", (int(w * 0.5) - 130, int(h * 0.92)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (150, 150, 150), 2, cv2.LINE_AA)
...
def run_practice_phase(screen_w, screen_h):
    ...
    checklist = SitToStandChecklist(leg_stand_threshold=LEG_STAND_THRESHOLD, leg_sit_threshold=LEG_SIT_THRESHOLD, back_min_angle=BACK_MIN_ANGLE, arm_raise_min=ARM_RAISE_MIN, arm_raise_max=ARM_RAISE_MAX)
    leg_state = 'standing'
    rep_count = 0
    correct_rep_count = 0
