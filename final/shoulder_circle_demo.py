...
INSTRUCTION_LINES = [
    "SHOULDER CIRCLE - How to do it:",
    "1. Stand straight with your arm hanging down",
    "2. Roll your shoulder/elbow in a smooth circle",
    "3. Keep the movement consistent in one direction",
    "4. Complete a full 360-degree rotation",
    "",
...
def draw_hud(image, exercise_name, rotation, reversal_count, direction, correct_reps, is_correct, latency_ms, fps):
    ...
    # Bottom-left: rotation reading
    if rotation is not None:
        dir_str = direction.upper() if direction else "NONE"
        cv2.putText(image, f"Rotation: {rotation:.1f} deg | Dir: {dir_str}", (20, h - 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(image, f"Reversals: {reversal_count}", (20, h - 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
    else:
...
def main():
    ...
    try:
        while True:
            ...
            draw_hud(display_image, EXERCISE_NAME, rotation, reversal_count, direction,
                     correct_reps, is_correct, avg_latency, live_fps)
